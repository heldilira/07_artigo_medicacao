...
#treemap(dtf = sum_df, vSize = "total_taken", index = "medicacao", vColor = "classe_medicacao", type = "categorical")

bar_plot_total <- ggplot(sum_df) +
  aes(
    x = medicacao,
...
sum_df |> 
  mutate(medicacao = fct_reorder(.x = medicacao, .fun = sum(total_taken))) |> 
  ggplot() +
  aes(
    x = medicacao,
    fill = classe_medicacao,
